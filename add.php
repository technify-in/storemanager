...
if(result){
  if (mysql_num_rows($result)!=1){
	...
	$qry3="SELECT `AUTO_INCREMENT`FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'store' AND   TABLE_NAME   = 'products';";
	$result3 = mysql_query($qry3) or die(mysql_error());
	$row=mysql_fetch_array($result3);

    $qry2="INSERT INTO `store`.`vbill` (`vid`, `vat_bill_id`, `bill_date`,`pid`, `xyz1`)".
    " VALUES ('$_REQUEST[seller]', '$_REQUEST[tiv]',  '$_REQUEST[dat]' , '$row[AUTO_INCREMENT]' , '23');";
